...
\large Easton, MD Town Council - Extended Remarks}
\hypersetup{
 pdfauthor={Norman Bauer, Director of GetTheFlockOuttaHere.org},
...
\section*{Extended Remarks Regarding Automated License Plate Readers in Easton}
\label{sec:orgef5f9eb}
...
\section*{Private property is not a jurisdictional escape hatch}
\label{sec:org6f62db5}
...
\section*{These are not ordinary security cameras}
\label{sec:org6b25f56}
...
\section*{A license plate may be visible, but visibility is not consent}
\label{sec:org841a7e5}
...
\section*{The system records the innocent by design}
\label{sec:org070ee88}
...
\section*{Regulation is not enough}
\label{sec:org4838452}
...
This is why we are not asking the Town merely to regulate automated license plate readers. We are demanding that the Town prohibit them.

\section*{Good intentions are not sufficient safeguards}
\label{sec:org79fa85e}
...
\section*{The warning is not new}
\label{sec:orga1347aa}
...
\section*{Consent was never requested}
\label{sec:orgfbe0bac}
...
You are right. The testimony should not describe the ordinance the Council ought to write. It should demand adoption of the ordinance already placed before them. That draft already contains the comprehensive prohibition, definitions, private-property application, enforcement provisions, exclusions for ordinary security cameras, data deletion requirements, and restrictions on indirect government access. 

Replace the existing section with this:

\section*{Our demand}
\label{sec:orga1a3432}
...
We demand that the Town Council introduce and adopt that ordinance.
...
We demand that a member of this Council introduce the proposed ordinance and that the Council hold the public legislative process necessary to bring it to a vote.

Do not replace a prohibition with a retention policy.

Do not substitute regulation for a ban.

Do not reduce this demand to a discussion of one vendor, one camera model, or one installation.
...
The cameras must be removed.

The surveillance must end.

The ordinance placed before this Council must be introduced, considered publicly, and adopted.

There is no acceptable compromise that leaves the infrastructure operating.

There is no acceptable regulation that converts an illegitimate system into a legitimate one.

There is no acceptable policy that permits the indiscriminate collection of the public’s movements so long as officials promise to use the information responsibly.

Remove the cameras.

Fulfill the Town’s duty to protect the people of Easton.
